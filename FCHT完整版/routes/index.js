...
router.post('/AdminLoginAndRegHandler', function (req, res) {
    ...
    switch (action) {
        case "login": // 登录
            ...
            handler("find", "administors", {userName: req.body.userName, password: password}, function (data) {
                if (data.length === 0) {
        ...
        case "returnuserinfo": // 返回登录管理员信息
            ...
            handler("find", "administors", {"_id": sessionId}, function (data) {
        ...
        case "add": // 添加管理员
            handler("find", "administors", {userName: req.body.userName}, function (docs) {
                if (docs.length != 0) {
                    ...
                    handler("find", "administors", null, function (arr) {
                        ...
                        if (userInfos.phone != false && userInfos.userName != false && userInfos.turename != false) {
                            handler("add", "administors", userInfos, function (data) {
                                if (data.result.n != 1) {
...
router.get('/AdminHandler', function (req, res) {
    ...
    switch (action) {
        ...
        case "delete": // 管理员删除
            //删除操作
            handler("deletes", "administors", {tokenId: parseInt(req.query.tokenId), isdelete: true}, function (data) {
                if (data.result.n == 0) {
                    res.send({"err": "删除失败"});
                } else {
                    //获取tokenId大于当前tokenId的数据集，并迭代处理管理员列表的tokenId
                    handler("find", "administors", {tokenId: {$gt: parseInt(req.query.tokenId)}}, function (da) {
...
router.post('/AdminHandler', function (req, res) {
    ...
    switch (action) {
        case "update": //修改管理员信息
            ...
            ;
            handler("updates", "administors", selectors, function (data) {
                if (data.result.n == 0) {
        ...
        case "getcourse": // 获取 具体的课程信息
            ...
            handler("find", "productList", {ID: parseInt(req.query.ID)}, function (data) {
                if (data.length == 0) {
                    ...
                    handler("find", "directoryList", {CDid: data[0]._id}, function (da) {
                        var result = {
                            success: "获取成功",
                            data: {
                                courselist: data[0],
...
router.post('/UpLoadPicHandler', function (req, res) {
    ...
    form.uploadDir = "temporary/";// 设置上传文件存放的文件夹
